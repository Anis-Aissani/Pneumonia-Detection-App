...
\begin{tikzpicture}[
  every node/.style={font=\small},
  class/.style={rectangle, draw, thick, minimum width=4cm, fill=white},
]
  % Classe User
  ...
  % Relations
  \draw[thick, -{Stealth[length=4pt]}] (user) -- node[above, font=\scriptsize]{crée} (session);
  \draw[thick, -{Stealth[length=4pt]}] (session) -- node[right, font=\scriptsize]{contient (0..*)} (pred);
  \draw[thick, -{Stealth[length=4pt]}] (user) -- node[left, font=\scriptsize]{effectue (0..*)} (pred);
\end{tikzpicture}
